...
func runCertsTest() {
	...
	start = time.Now()
	updatedDomainFromDB, err := dnConn.RetrieveUpdatedDomainMultiThread(ctx, 1000)
	if err != nil {
		panic(err)
	}
...
func runPCAndRPCTest() {
	...
	// get rpc and pc.
	pcList, rpcList, err := logpicker.GetPCAndRPC("./config/top_10k_domains/domains.txt", 0, 0, 0)
	if err != nil {
		panic(err)
	}
	...
	fmt.Println("time to update ", len(pcList)+len(rpcList), " RPC and PC ", end.Sub(start))

	ctx, cancelF := context.WithTimeout(context.Background(), time.Minute)
	defer cancelF()

	start = time.Now()
	_, err = dnConn.RetrieveUpdatedDomainMultiThread(ctx, 1000)
	if err != nil {
		panic(err)
	}
