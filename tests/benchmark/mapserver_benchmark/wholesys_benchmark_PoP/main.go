...
func main() {
	...
	if len(testSet) != 100038 {
		panic("read file error")
	}

	/*
		testSet = testSet[0:50000]

		for i := 0; i < 100000; i++ {
			testSet = append(testSet, RandStringRunes(30))
		}*/
...
func fetchProofs(root []byte, names []string, csv *csv.Writer) {
	ctx, cancelF := context.WithTimeout(context.Background(), 10*time.Minute)
	defer cancelF()
