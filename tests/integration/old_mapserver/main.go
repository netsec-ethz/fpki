...
func main() {
	...
	mapResponder, err := responder.NewOldMapResponder(ctx, root, 233, "./config/mapserver_config.json")
	if err != nil {
		panic(err)
	}

	// re-collect the added certs
	collectedCertList := []ctX509.Certificate{}
	for i := 0; i < 50; i++ {
		...
		collectedCertList = append(collectedCertList, certList...)
	}

	numberOfWorker := 15
	wg.Add(numberOfWorker)
	step := len(collectedCertList) / numberOfWorker

	for i := 0; i < numberOfWorker; i++ {
		worker(ctx, collectedCertList[i*step:i*step+step-1], mapResponder)
	}
...
func worker(ctx context.Context, certs []ctX509.Certificate, mapResponder *responder.OldMapResponder) {
