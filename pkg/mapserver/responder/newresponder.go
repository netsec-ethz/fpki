...
type UpdatedMapResponder struct {
	workerPool []*responderWorker
	workerChan chan ClientRequest
}

func NewUpdatedMapResponder(root []byte, cacheHeight int, workerThreadNum int) (*UpdatedMapResponder, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("NewUpdatedMapResponder | LoadCache | %w", err)
	}
	...
	return &UpdatedMapResponder{
		workerChan: clientInputChan,
		workerPool: workerPool,
	}, nil
}

func (responder *UpdatedMapResponder) GetProof(ctx context.Context, domainName string) ([]mapCommon.MapServerResponse, error) {
