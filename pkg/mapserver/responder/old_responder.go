...
// OldMapResponder: A map responder, which is responsible for receiving client's request. Only read from db.
type OldMapResponder struct {
	conn            db.Conn
...
func (r *OldMapResponder) loadPrivKeyAndSignTreeHead(mapServerConfigPath string) error {
...
func (r *OldMapResponder) GetSignTreeHead() []byte {
	return r.signedTreeHead
}

func newMapResponder(conn db.Conn, smt *trie.Trie) *OldMapResponder {
	return &OldMapResponder{
		conn:            conn,
...
func (r *OldMapResponder) GetProof(ctx context.Context, domainName string) ([]mapCommon.MapServerResponse, error) {
...
func (mapResponder *OldMapResponder) GetRoot() []byte {
	return mapResponder.smt.Root
}

// Close: close db
func (mapResponder *OldMapResponder) Close() error {
...
func (mapResponder *OldMapResponder) getProofFromSMT(ctx context.Context,
	domainMap map[common.SHA256Output]*mapCommon.MapServerResponse,
