...
func (s *SMT) MerkleProof(key []byte) ([][]byte, error) {
	...
	return s.merkleProof(s.Root, TreeHeight, 0, key, nil)
}
...
func (s *SMT) MerkleProofCompressed(key []byte) ([]byte, [][]byte, error) {
	s.lock.RLock()
	defer s.lock.RUnlock()
	bitmap := make([]byte, TreeHeight/8)
	mp, err := s.merkleProofCompressed(s.Root, 256, 0, key, bitmap, nil)
...
func (s *SMT) MerkleProofCompressed2(key []byte) ([]byte, [][]byte, error) {
	...
	mpFull, err := s.merkleProof(s.Root, 256, 0, key, nil)
	if err != nil {
		return nil, nil, err
	}
	var mp [][]byte
...
func VerifyMerkleProof(root []byte, ap [][]byte, key, value []byte) bool {
	return bytes.Equal(root, verifyMerkleProof(ap, 256, key, value))
}
...
func (s *SMT) VerifyMerkleProofCompressed(bitmap []byte, ap [][]byte, key, value []byte) bool {
	return bytes.Equal(s.Root, s.verifyMerkleProofCompressed(bitmap, ap, TreeHeight, len(ap), key, value))
}

// verifyMerkleProof verifies that a key/value is included in the trie with given root
func verifyMerkleProof(ap [][]byte, height int, key, value []byte) []byte {
	if height == 0 {
		return value
	}
	if bitIsSet(key, 256-height) {
		return Hasher(ap[height-1], verifyMerkleProof(ap, height-1, key, value))
	}
	return Hasher(verifyMerkleProof(ap, height-1, key, value), ap[height-1])
}
...
func (s *SMT) unrollShortcutAndKey(key, value []byte, height int, emptyKey []byte) [][]byte {
	...
	if bitIsSet(key, TreeHeight-height) == bitIsSet(emptyKey, TreeHeight-height) {
