...
func UpdateDomainEntries(certs []*x509.Certificate, dbConn db.Conn, readerNum int) (int, error) {
	if len(certs) == 0 {
		return 0, nil
	}
	...
	updatedDomains, err := updateDomainEntries(domainEntriesMap, certs, domainCertMap)
	if err != nil {
		return 0, fmt.Errorf("UpdateDomainEntries | updateDomainEntries | %w", err)
	}

	// commit changes to db
	return writeChangesToDB(updatedDomains, domainEntriesMap, dbConn)
}
...
func updateDomainEntries(domainEntries map[string]*common.DomainEntry, certs []*x509.Certificate, certDomainMap map[string][]*x509.Certificate) (map[string]byte, error) {
