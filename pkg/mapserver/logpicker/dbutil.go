...
func writeChangesToDB(updatedDomains map[string]byte, domainEntriesMap map[string]*common.DomainEntry, dbConn db.Conn) (int, error) {
	...
	go writeToDomainEntriesTable(ctx, dbConn, keyValuePairs, resultChan)
	// write changes to updates table
	go writeToUpdateTable(ctx, dbConn, updatedDomainNames, resultChan)
