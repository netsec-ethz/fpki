...
type PL_LogServer struct {
	config *PL_LogServerConfig
}

func PL_CreateLogServer(configPath string) {
	flag.Parse()

	logConfig := &PL_LogServerConfig{}
	pl_LogServer := &PL_LogServer{
		config: logConfig,
	}
	...
	unannounce := AnnounceSelf(ctx, client, pl_LogServer.config.EtcdService, pl_LogServer.config.RpcEndpoint, cancel)
	defer unannounce()
